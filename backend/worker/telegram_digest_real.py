#!/usr/bin/env python3
"""
Real Telegram Digest - Sử dụng data thực từ database
"""
...
class RealTelegramDigest:
    def __init__(self):
        ...
        logger.info(f"Real Telegram Digest initialized")
    ...
    def _get_real_signals(self) -> list:
        ...
        signals_data.sort(key=lambda x: (x['signal_strength'], x['confidence']), reverse=True)
    ...
    def _analyze_signals(self, signals, ticker, company_name, exchange) -> dict:
        ...
        total_confidence = sum(s[2] or 0 for s in signals)
        avg_confidence = total_confidence / len(signals) if signals else 0
        
        # Determine primary signal
        if len(confirmed_signals) >= 2:
            primary_signal = 'CONFIRMED'
            signal_type = 'confirmed_bullish' if len([s for s in confirmed_signals if 'bullish' in s[1]]) > len([s for s in confirmed_signals if 'bearish' in s[1]]) else 'confirmed_bearish'
        ...
        # Get best risk metrics
        best_signal = max(confirmed_signals, key=lambda x: x[2] or 0) if confirmed_signals else signals[0]
        ...
        return {
            ...
            'risk': best_signal[4] or 'MED',
            'rr_ratio': round(best_signal[7] or 1.5, 1),
            'price': round(best_signal[5] or 0, 2) if best_signal[5] else 0,
    ...
    def _generate_reasoning(self, signals, primary_signal) -> str:
        ...
        if primary_signal == 'CONFIRMED':
            return f"✅ Confirmed: {', '.join(confirmed_tf)} | 🎯 Multi-timeframe confirmation"
        elif primary_signal in ['BULLISH', 'BEARISH']:
            return f"📊 Trend alignment | 🎯 {len(signals)} timeframes analyzed"
        else:
            return f"⚪ Neutral trend | 📊 {len(signals)} timeframes analyzed"
    ...
    def _format_telegram_table(self, signals_data: list) -> str:
        ...
        # Table header
        message += f"{'#':<2} {'Symbol':<6} {'Signal':<10} {'Conf':<5} {'Risk':<4} {'R/R':<4} {'Price':<8} {'Analysis':<15}\n"
        message += "─" * 70 + "\n"
        ...
        for i, data in enumerate(signals_data[:30], 1):
            ...
            message += f"{i:<2} {symbol:<6} {signal_icon}{signal:<9} {conf:<5} {risk_icon}{risk:<3} {rr:<4} {price:<8} {analysis:<15}\n"
        ...
        for i, data in enumerate(signals_data[:5], 1):
            ...
            message += f"📈 R/R Ratio: {data['rr_ratio']:.1f} | 🎯 {data['reasoning']}\n\n"
    ...
    def run_loop(self):
        """Run in a loop"""
        logger.info(f"Starting Real Telegram Digest loop; interval={self.interval} seconds")
